...
$.widget("custom.editWindow", {
    ...
    loadSong: function() {
        ...
        if(this.currentSongID == -1) {
            ...
            this._trigger("callback", null, { songID: this.currentSongID,
                callback: function(data) { ownElement.editWindow("songLoaded", data["results"][0]) } });
        }
    },
    ...
    songLoaded: function(data) {
        ...
        ownElement.find("#title_input").completion("value", data.song_title);
	...
	_create: function() {
		...
		inputElement.autocomplete({
			...
			source: function(request, responseFunction) {
				...
				$.getJSON("/db/completion", {type: column, term: request.term},  function(data) {
                    console.log(data["results"]);
                    responseFunction(data["names"]);
                });
			},
			minLength: 1,
